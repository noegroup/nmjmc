...
class RepulsiveParticles(object):
    ...
    def init_positions(self, dimer_distance, scaling_factor=1.05):
        ...
        locs = np.linspace(
            -self.params["box_halfsize"] - 1, self.params["box_halfsize"] + 1, sqrtn + 2
        )[1:-1]
    ...
    def LJ_energy(self, x):
        ...
        # energy
        E = (
            0.5 * self.params["eps"] * np.sum(D2rel ** 6, axis=(1, 2))
    ...
    def LJ_energy_tf(self, x):
        ...
        # energy
        E = (
            0.5 * self.params["eps"] * tf.reduce_sum(D2rel ** 6, axis=(1, 2))
        )  # do 1/2 because we have double-counted each interaction
        return E
    ...
    def LJ_energy_hybr_tf(self, x, De=1.0, a_tem=6.0):
        ...
        coeff = 25
        D2rel = tf.where(
            D2 > coeff * (0.1 ** cut_exp),
            (self.params["rm"] ** 2) / D2,
            (10 ** cut_exp) / coeff
            + coeff * (10 ** (slope_exp - cut_exp))
            - 10 ** slope_exp * D2,
        )
        # remove self-interactions and interactions between dimer particles
        D2rel = D2rel * mmatrix
        ...
        #        E = 0.5 * self.params['eps'] * tf.where (D2<10, D2rel ** 6, 10**6 + D2rel )
        #        E_LJ = 0.5 * tf.reduce_sum(self.params['eps'] * (D2rel ** 6), axis=(1, 2))
        #        R = tf.sqrt(D2 + 0.001)  # add a little bit to prevent the gradient from blowing up
        #        E_morse = tf.reduce_sum((De * (1. - tf.exp(-a * (R - self.params['rm']))) ** 2 - De), axis=(1, 2))

        #        E = tf.where(E_LJ<1000000, E_LJ, E_morse)
    ...
    def box_energy(self, x):
        ...
        E += np.sum(
            (np.sign(d_right) + 1) * self.params["box_k"] * d_right ** 2, axis=1
        )
    ...
    def box_energy_tf(self, x):
        ...
        d_left = -(xcomp + self.params["box_halfsize"])
        E += tf.reduce_sum(
            (tf.sign(d_left) + 1) * self.params["box_k"] * d_left ** 2, axis=1
        )
        d_right = xcomp - self.params["box_halfsize"]
        E += tf.reduce_sum(
            (tf.sign(d_right) + 1) * self.params["box_k"] * d_right ** 2, axis=1
        )
        d_down = -(ycomp + self.params["box_halfsize"])
        E += tf.reduce_sum(
            (tf.sign(d_down) + 1) * self.params["box_k"] * d_down ** 2, axis=1
        )
        d_up = ycomp - self.params["box_halfsize"]
        E += tf.reduce_sum(
            (tf.sign(d_up) + 1) * self.params["box_k"] * d_up ** 2, axis=1
        )
        return E
    ...
    def grid_energy_tf(self, x):
        d2 = (x - self.grid) ** 2
        E = tf.reduce_sum(
            self.params["grid_k"] * (self.params["rm"] ** 2 * d2) ** 6, axis=1
        )
        return E
    ...
    def energy_hybr_tf(self, x):
        return (
            self.LJ_energy_hybr_tf(x) + self.dimer_energy_tf(x) + self.box_energy_tf(x)
        )

    def plot_dimer_energy(self, axis=None, **kwargs):
        ...
        axis.plot(x_scan, E_scan, linewidth=2, **kwargs)
